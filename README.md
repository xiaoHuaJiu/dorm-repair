...
当前仓库已建立前后端分离技术骨架，并完成用户注册、基础配置、三角色工单查询以及学生报修创建后端能力。自动派单、接单、维修、请假、评价等后续工单业务尚未实现，也未完整迁移 `prototype/` 页面。
...
阶段 1 建立了 17 张核心业务表，阶段 4 增加统一接口幂等记录表，阶段 5 增加节假日配置表和自动派单失败告警表，当前共 20 张正式表。现有后端包含 JWT 登录、三角色授权、基础配置、工单查询详情、学生报修创建及事务提交后异步自动派单能力。

当前已实现三角色工单分页查询、真实行级数据范围、四级位置树、工单详情聚合、24 小时疑似重复检查以及 Redis + MySQL 双层幂等的学生报修写入。仍未实现对应前端报修页面、自动派单算法、接单与维修写入、请假、转派、评价写入、返工写入、提醒和异常工单中心。
