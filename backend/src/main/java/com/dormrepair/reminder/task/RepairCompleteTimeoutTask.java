...
public class RepairCompleteTimeoutTask {private final RepairReminderService service;public RepairCompleteTimeoutTask(RepairReminderService service){this.service=service;}@Scheduled(cron="${app.task.timeout-scan-cron:0 * * * * ?}")public void scan(){service.createCompleteTimeoutReminders(LocalDateTime.now());}}
