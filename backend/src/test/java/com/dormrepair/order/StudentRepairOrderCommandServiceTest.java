...
class StudentRepairOrderCommandServiceTest {
  RepairOrderMapper orders; RepairEvaluationMapper evaluations; RepairReworkRecordMapper reworks; RepairOrderAlertMapper alerts; RepairOrderFlowMapper flows; SysOperationLogMapper logs; StudentRepairOrderTransactionService service;
  @BeforeEach void setup(){orders=mock(RepairOrderMapper.class);evaluations=mock(RepairEvaluationMapper.class);reworks=mock(RepairReworkRecordMapper.class);alerts=mock(RepairOrderAlertMapper.class);flows=mock(RepairOrderFlowMapper.class);logs=mock(SysOperationLogMapper.class);service=new StudentRepairOrderTransactionService(orders,evaluations,reworks,alerts,flows,logs,new ObjectMapper());LoginUser u=new LoginUser(7L,"student01","学生",UserRoleEnum.STUDENT);SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken(u,null,List.of()));}
