...
class WorkerRepairOrderCommandServiceTest {
    ...
    @BeforeEach void setUp() {
        ...
        flows=mock(RepairOrderFlowMapper.class); logs=mock(SysOperationLogMapper.class); resolver=mock(CurrentWorkerResolver.class);
        service=new WorkerRepairOrderCommandService(orders,processes,materials,flows,logs,resolver,new ObjectMapper());
    ...
    @Test void submitResultShouldPersistRecordAndTransition() {
        when(orders.selectByIdForUpdate(1L)).thenReturn(order(4,10L)); when(orders.casSubmitResult(eq(1L),eq(10L),any())).thenReturn(1);
